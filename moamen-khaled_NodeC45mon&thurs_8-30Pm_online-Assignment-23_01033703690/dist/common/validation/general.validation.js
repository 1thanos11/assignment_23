...
import { MediaTypeEnum, PostStatusEnum, PostVisibilityEnum, } from "../enums/post.enums.js";
...
export const generalValidationFields = {
    ...
    date: z.date(),
    string: (min, max) => z.string().min(min).max(max),
    email: z.email(),
    ...
    nullString: z.string(),
    countryCode: z.custom((val) => {
        return (typeof val === "string" && getCountries().includes(val));
    }, {
        message: "invalid country code",
    }),
    otp: z.string().length(6),
    logoutFlag: z.enum(LogoutFlagEnum),
    file: (allowedMimeTypes, maxSizeMB = 10) => z.object({
        fieldname: z.string(),
    ...
    files: (allowedMimeTypes, maxSizeMB = 10, maxCount = 10) => z
        .array(z.object({
        fieldname: z.string(),
        originalname: z.string(),
        encoding: z.string(),
        ...
        .max(maxCount, `Max ${maxCount} files allowed`),
};
