...
class InvitationProcess {
    ...
    async sendInvitationMessages(invitations) {
        await Promise.all(invitations.map(async (invitation) => {
            try {
                ...
                if (!ovoChat) {
                    console.error(`Failed to find or create chat for invitation ${invitation.invitationId}`);
                    return;
                }
                ...
                if (!message) {
                    console.error(`Failed to create message for invitation ${invitation.invitationId}`);
                    return;
                }
                const socketIds = await this.redis.sMembers(this.redis.socketKey(invitation.receiverId));
                if (socketIds.length) {
                    this.realtime.getIo.to(socketIds).emit("newMessage", message);
                }
            }
            catch (error) {
                console.error(`Error processing invitation ${invitation.invitationId}:`, error);
            }
        }));
    }
}
