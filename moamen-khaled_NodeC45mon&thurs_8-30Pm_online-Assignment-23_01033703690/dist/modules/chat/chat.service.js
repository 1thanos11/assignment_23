...
import { ProfileRepository, SettingsRepository, UserRepository, } from "../../infra/repository/index.js";
...
class ChatService {
    ...
    async createGroup(inputs) {
        ...
        if (notAllowedUsers.length > 0) {
            ...
            if (invitations.length) {
                await invitationQueue.add(JobEnum.CREATE_INVITATION_MESSAGES, {
    ...
    async sendOVMMessage(inputs) {
        ...
        if (!chat)
            throw new NotFoundError("Group chat not found");
        const isParticipant = chat.participants.some((p) => p.toString() === user._id.toString());
        if (!isParticipant)
            throw new ForbiddenError("You are not in this group");
        const message = await this.messageRepository.createOne({
            data: {
                chatId: chat._id,
                senderId: user._id,
        ...
        await Promise.all(otherParticipants.map(async (participantId) => {
            const socketIds = await this.redis.sMembers(this.redis.socketKey(participantId));
        ...
        return { roomId: chat.roomId, message };
    }
}
