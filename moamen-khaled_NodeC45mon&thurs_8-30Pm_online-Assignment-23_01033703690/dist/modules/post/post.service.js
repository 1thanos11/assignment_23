...
import { FollowRepository, NotificationRepository, ProfileRepository, UserRepository, } from "../../infra/repository/index.js";
...
class PostService {
    ...
    postProjection = "content media commentsCount sharesCount likesCount viewsCount createdAt";
    async createPost(inputs) {
        ...
        if (files && files.length) {
            void s3Queue.add(JobEnum.UPLOAD_POST_MEDIA, { userId: user._id, files, folderId: post.folderId }, {
                attempts: 3,
    ...
    async updatePost(inputs) {
        const { user, postId, content, files, removeFiles, tags, removeTags, postVisibility, } = inputs;
        ...
        if ((tags && tags.length && !profile) || !post) {
            throw new NotFoundError(`Something went wrong may be post not found or you don't have profile`);
        }
        if (tags && users.length !== tags.length) {
            throw new NotFoundError(`Fail to find mention accounts`);
        }
        ...
        if ((files && files.length) || (removeFiles && removeFiles.length)) {
            void s3Queue.add(JobEnum.UPDATE_POST_MEDIA, {
                userId: user._id,
                postId: post._id,
    ...
    async postsList(inputs) {
        ...
        if (user._id.equals(targetUserId)) {
            ...
            const posts = await this.redis.cache({
                key,
                ttl,
                fn: () => this.postRepository.paginate({
                    filter: {
                        authorId: user._id,
    ...
    async getPostById(inputs) {
        ...
        if (user._id.equals(targetUserId)) {
            ...
            const post = await this.redis.cache({
                key,
                ttl: CacheTTL.POST,
                fn: () => this.postRepository.findOne({
