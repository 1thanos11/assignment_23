...
class SettingsService {
    ...
    async updateSettings(inputs) {
        const { user, profileVisibility, showOnLineStatus, showLastSeen, showEmail, showPhone, showLocation, showDOB, showJoinedAt, showEducation, showRelation, showFollowersList, showFollowingsList, language, showInSearch, showInRecommendations, allowNotifications, allowGroupAdding, } = inputs;
        const update = {};
        ...
        if (showInSearch !== undefined)
            update.showInSearch = showInSearch;
    ...
    async getSettings(inputs) {
        ...
        const settings = await this.redis.cache({
            ...
            fn: () => this.settingsRepository.findOne({ filter: { ownerId: user._id } }),
        });
        if (!settings) {
            throw new NotFoundError(`Settings not found`);
        }
